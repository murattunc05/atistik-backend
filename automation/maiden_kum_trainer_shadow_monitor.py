"""Fail-closed prospective monitor for MAIDEN|Kum trainer-score +2."""
...
EXPECTED_VERSION = "maiden-kum-trainer-plus2-20260823-v1"
EXPECTED_OBSERVATION_START = "23.08.2026"
EXPECTED_BASELINE_VERSION = "4.25"
EXPECTED_PROFILE = "MAIDEN|Kum"
...
AUDIT_PATH = (
    ...
    / "evidence"
    / "maiden_kum_trainer_plus2_audit_20260822.json"
)
MANIFEST_PATH = (
    Path(__file__).resolve().parent
    / "evidence"
    / "maiden_kum_trainer_plus2_manifest_20260822.json"
)
EXPECTED_CALIBRATION_CONTRACT = {
    ...
    "auditArtifact": EXPECTED_AUDIT_ARTIFACT,
    "auditArtifactSha256": EXPECTED_AUDIT_SHA256,
    "manifestArtifact": EXPECTED_MANIFEST_ARTIFACT,
    "manifestArtifactSha256": EXPECTED_MANIFEST_SHA256,
...
def _identity_valid(rows: list[dict[str, Any]]) -> bool:
    ...
    common_keys = (
        ...
        "distance",
        "field_size",
        "maiden_kum_trainer_candidate_version",
        "maiden_kum_trainer_candidate_observation_start",
        "maiden_kum_trainer_candidate_created_ts",
        "maiden_kum_trainer_candidate_baseline_version",
    )
    for key in common_keys:
        ...
        if len(values) != 1 or not next(iter(values)):
            return False
    common_objects = (
        "maiden_kum_trainer_candidate_profile",
        "maiden_kum_trainer_candidate_baseline_weights",
        "maiden_kum_trainer_candidate_raw_weights",
        "maiden_kum_trainer_candidate_weights",
        "maiden_kum_trainer_candidate_weight_delta_pct",
        "maiden_kum_trainer_candidate_definition_sha256",
        "maiden_kum_trainer_candidate_calibration_contract",
        "maiden_kum_trainer_candidate_race_snapshot_sha256",
    )
    ...
    if safe_int(rows[0].get("field_size"), 0) != len(rows):
        return False
    if (
        str(rows[0].get("track") or "").casefold() != "kum"
        or "maiden" not in str(rows[0].get("race_type") or "").casefold()
    ):
        return False

    race_day = parse_race_date(rows[0].get("race_date"))
    race_time = str(rows[0].get("race_time") or "").strip().replace(".", ":")
    created_ts = safe_int(rows[0].get("maiden_kum_trainer_candidate_created_ts"), 0)
...
def _definition_valid(rows: list[dict[str, Any]]) -> bool:
    first = rows[0]
    profile = first.get("maiden_kum_trainer_candidate_profile")
    formula = expected_formula(first.get("maiden_kum_trainer_candidate_baseline_weights"))
    if (
        not isinstance(profile, dict)
        or profile.get("category") != "MAIDEN"
        or str(profile.get("track") or "").casefold() != "kum"
        or not str(profile.get("selectedKey") or "")
        or formula is None
    ):
        return False
    observed_raw = numeric_weights(first.get("maiden_kum_trainer_candidate_raw_weights"))
    observed_weights = numeric_weights(first.get("maiden_kum_trainer_candidate_weights"))
    observed_delta = first.get("maiden_kum_trainer_candidate_weight_delta_pct")
    if (
        ...
        or abs(
            safe_float(first.get("maiden_kum_trainer_candidate_baseline_raw_total"), -1.0)
            - formula["baselineRawTotal"]
        ) > 1e-8
        or abs(
            safe_float(first.get("maiden_kum_trainer_candidate_raw_total"), -1.0)
    ...
    for row in rows:
        ...
        if (
            str(row.get("maiden_kum_trainer_candidate_version") or "") != EXPECTED_VERSION
            or str(row.get("maiden_kum_trainer_candidate_mode") or "") != EXPECTED_MODE
            or str(row.get("maiden_kum_trainer_candidate_observation_start") or "")
            != EXPECTED_OBSERVATION_START
            or str(row.get("maiden_kum_trainer_candidate_baseline_version") or "")
            != EXPECTED_BASELINE_VERSION
            or str(row.get("v4_version") or "") != EXPECTED_BASELINE_VERSION
            or str(row.get("maiden_kum_trainer_candidate_metric") or "") != EXPECTED_METRIC
            or abs(
                safe_float(row.get("maiden_kum_trainer_candidate_raw_weight_add_points"), -1.0)
                - EXPECTED_RAW_ADD_POINTS
            ) > 1e-9
            or bool(row.get("maiden_kum_trainer_candidate_used_for_ranking"))
            or bool(row.get("maiden_kum_trainer_candidate_telegram_visible"))
            or bool(row.get("maiden_kum_trainer_candidate_rollout_eligible"))
            or not bool(row.get("maiden_kum_trainer_candidate_formal_replay_only"))
            ...
            or str(row.get("maiden_kum_trainer_candidate_definition_sha256") or "")
            != expected_definition_hash
            or canonical(
                row.get("maiden_kum_trainer_candidate_calibration_contract")
            )
            != canonical(EXPECTED_CALIBRATION_CONTRACT)
            or str(row.get("maiden_kum_trainer_candidate_audit_artifact_sha256") or "")
            != EXPECTED_AUDIT_SHA256
            or str(row.get("maiden_kum_trainer_candidate_manifest_artifact_sha256") or "")
            != EXPECTED_MANIFEST_SHA256
            or str(row.get("maiden_kum_trainer_candidate_source_snapshot_sha256") or "")
            != EXPECTED_SOURCE_SNAPSHOT_SHA256
            or row.get("maiden_kum_trainer_candidate_ml15_overlap_non_additive") is not True
        ):
            return False
    return True
...
def _source_and_feature_valid(rows: list[dict[str, Any]]) -> bool:
    ...
    neutral_count = 0
    for row in rows:
        source = row.get("maiden_kum_trainer_candidate_source")
        snapshot = row.get("maiden_kum_trainer_candidate_feature_snapshot")
        guards = row.get("maiden_kum_trainer_candidate_source_guard_snapshot")
        ...
        if (
            ...
            or source.get("metricSourceFlag") != "hasTrainer"
            or source.get("mfGuard") != "_has_trainer"
            ...
            or flags.get("hasTrainer") != source.get("metricSourceFlagValue")
        ):
            return False
        metric_guard = guards.get("_has_trainer")
        if (
        ...
        feature_payload = {
            ...
            "features": snapshot,
            "sourceGuards": guards,
            "trainerSource": source_for_hash,
        }
        if str(row.get("maiden_kum_trainer_candidate_feature_vector_sha256") or "") != sha256(feature_payload):
            return False

    runner_count = len(rows)
    unavailable_count = runner_count - source_count
    for row in rows:
        source = row.get("maiden_kum_trainer_candidate_source") or {}
        if (
...
def _score_and_rank_valid(rows: list[dict[str, Any]]) -> bool:
    first_weights = numeric_weights(rows[0].get("maiden_kum_trainer_candidate_weights"))
    formula = expected_formula(rows[0].get("maiden_kum_trainer_candidate_baseline_weights"))
    ...
    for index, row in enumerate(rows):
        baseline_score = finite(row.get("maiden_kum_trainer_candidate_baseline_score"))
        baseline_rank = safe_int(row.get("maiden_kum_trainer_candidate_baseline_rank"), 0)
        replay_baseline_rank = safe_int(
            row.get("maiden_kum_trainer_candidate_replay_baseline_rank"),
            0,
        )
        base_score = finite(row.get("maiden_kum_trainer_candidate_base_score"))
        penalty = finite(row.get("maiden_kum_trainer_candidate_penalty_total"))
        score = finite(row.get("maiden_kum_trainer_candidate_score"))
        rank = safe_int(row.get("maiden_kum_trainer_candidate_rank"), 0)
        snapshot = row.get("maiden_kum_trainer_candidate_feature_snapshot")
        guards = row.get("maiden_kum_trainer_candidate_source_guard_snapshot")
        components = row.get("maiden_kum_trainer_candidate_score_components")
        if (
        ...
        source = row.get("maiden_kum_trainer_candidate_source") or {}
        added_value = finite(row.get("maiden_kum_trainer_candidate_added_metric_value"))
        if bool(source.get("hasSource")):
            if added_value is None or abs(added_value - finite(snapshot.get(EXPECTED_METRIC))) > 1e-9:
                return False
        elif row.get("maiden_kum_trainer_candidate_added_metric_value") is not None:
            return False
        ...
        if (
            abs(
                safe_float(row.get("maiden_kum_trainer_candidate_baseline_weighted_numerator"), -1.0)
                - baseline_numerator
            ) > 1e-8
            or abs(
                safe_float(row.get("maiden_kum_trainer_candidate_baseline_available_weight_total"), -1.0)
                - baseline_denominator
            ) > 1e-8
            or abs(
                safe_float(row.get("maiden_kum_trainer_candidate_replay_baseline_base_score"), -1.0)
                - expected_replay_baseline_base
            ) > 1e-8
            or abs(
                safe_float(row.get("maiden_kum_trainer_candidate_replay_baseline_score"), -1.0)
                - expected_replay_baseline_score
            ) > 1e-8
            or abs(
                safe_float(row.get("maiden_kum_trainer_candidate_weighted_numerator"), -1.0)
                - candidate_numerator
            ) > 1e-8
            or abs(
                safe_float(row.get("maiden_kum_trainer_candidate_available_weight_total"), -1.0)
    ...
    baseline_ranks = [safe_int(row.get("maiden_kum_trainer_candidate_baseline_rank"), 0) for row in rows]
    replay_baseline_ranks = [
        safe_int(row.get("maiden_kum_trainer_candidate_replay_baseline_rank"), 0)
        for row in rows
    ]
    candidate_ranks = [safe_int(row.get("maiden_kum_trainer_candidate_rank"), 0) for row in rows]
    if (
    ...
    actionable_count = safe_int(
        (rows[0].get("maiden_kum_trainer_candidate_source") or {}).get(
            "actionableCount"
        ),
        0,
    )
    # Pre-race eligibility requires source actionability and a compatible
    # visible/replay baseline. Final compatibility is re-evaluated after
    # official non-runners are filtered.
    expected_evidence_eligible = bool(
        actionable_count > 0 and replay_top3_set_agreement
    )
    for row in rows:
        if (
            row.get("maiden_kum_trainer_candidate_replay_top3_set_agreement")
            is not replay_top3_set_agreement
            or row.get("maiden_kum_trainer_candidate_evidence_issue")
            != expected_issue
            or row.get("maiden_kum_trainer_candidate_race_evidence_eligible")
            is not expected_evidence_eligible
        ):
            return False

    definition_hash = str(rows[0].get("maiden_kum_trainer_candidate_definition_sha256") or "")
    created_ts = safe_int(rows[0].get("maiden_kum_trainer_candidate_created_ts"), 0)
    race_payload = {
        ...
        "horses": sorted(
            [
                {
                    "horseName": str(row.get("horse_name") or "").strip(),
                    "baselineScore": row.get("maiden_kum_trainer_candidate_baseline_score"),
                    "baselineRank": row.get("maiden_kum_trainer_candidate_baseline_rank"),
                    "replayBaselineScore": row.get(
                        "maiden_kum_trainer_candidate_replay_baseline_score"
                    ),
                    "replayBaselineRank": row.get(
                        "maiden_kum_trainer_candidate_replay_baseline_rank"
                    ),
                    "candidateScore": row.get("maiden_kum_trainer_candidate_score"),
                    "candidateRank": row.get("maiden_kum_trainer_candidate_rank"),
                    "featureVectorSha256": row.get("maiden_kum_trainer_candidate_feature_vector_sha256"),
                }
                for row in rows
            ],
    ...
    return all(
        str(row.get("maiden_kum_trainer_candidate_race_snapshot_sha256") or "")
        == expected_race_hash
        for row in rows
    )
...
def calibration_evidence_status() -> dict[str, Any]:
    """Verify both frozen audit and manifest before opening calibration."""
    base = {
        "ready": False,
        "auditArtifact": EXPECTED_AUDIT_ARTIFACT,
        "expectedAuditSha256": EXPECTED_AUDIT_SHA256,
        "auditSha256": None,
        "manifestArtifact": EXPECTED_MANIFEST_ARTIFACT,
    ...
    def read_pinned(path: Path, expected_hash: str, label: str):
        try:
            raw = path.read_bytes()
        ...
        if actual_hash != expected_hash:
            return None, actual_hash, f"{label}_sha256_mismatch"
        try:
            payload = json.loads(raw.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError):
            return None, actual_hash, f"{label}_json_invalid"
        if not isinstance(payload, dict):
            return None, actual_hash, f"{label}_schema_mismatch"
        return payload, actual_hash, None

    audit, audit_hash, error = read_pinned(
        AUDIT_PATH, EXPECTED_AUDIT_SHA256, "audit_artifact"
    )
    current = {**base, "auditSha256": audit_hash}
    if error:
        return {**current, "reason": error}
    manifest, manifest_hash, error = read_pinned(
        MANIFEST_PATH, EXPECTED_MANIFEST_SHA256, "manifest_artifact"
    )
    current = {**current, "manifestSha256": manifest_hash}
    if error:
        return {**current, "reason": error}

    audit_scope = audit.get("scope") or {}
    chronology = audit.get("chronology") or {}
    audit_source = audit.get("source") or {}
    overlap = audit.get("maidenMl15Overlap") or {}
    calibration = audit.get("calibration") or {}
    audit_checks = (
        audit.get("schemaVersion") == "maiden-kum-trainer-audit-v1",
        audit.get("candidateVersion") == EXPECTED_VERSION,
        audit.get("decision") == "GO_PROSPECTIVE_SHADOW_ONLY",
        audit.get("live") is False,
        audit.get("usedForRanking") is False,
        audit.get("telegramVisible") is False,
        audit_scope.get("category") == "MAIDEN",
        str(audit_scope.get("track") or "").casefold() == "kum",
        audit_scope.get("baselineVersion") == EXPECTED_BASELINE_VERSION,
        audit_scope.get("metric") == EXPECTED_METRIC,
        audit_scope.get("rawWeightAddPoints") == EXPECTED_RAW_ADD_POINTS,
        audit_scope.get("observationStart") == EXPECTED_OBSERVATION_START,
        audit_source.get("sha256") == EXPECTED_SOURCE_SNAPSHOT_SHA256,
        audit_source.get("validDateOnly") is True,
        (chronology.get("build") or {}).get("races") == 19,
        (chronology.get("inner") or {}).get("races") == 6,
        (chronology.get("outer") or {}).get("races") == 7,
        (chronology.get("outer") or {}).get("untouched") is False,
        overlap.get("nonAdditive") is True,
        calibration.get("temperature") == EXPECTED_BASELINE_TEMPERATURE,
        calibration.get("fitScope") == "build_only",
    )
    manifest_scope = manifest.get("scope") or {}
    manifest_formula = manifest.get("formula") or {}
    manifest_audit = manifest.get("auditEvidence") or {}
    manifest_overlap = manifest.get("overlapPolicy") or {}
    manifest_gates = manifest.get("gates") or {}
    manifest_checks = (
        manifest.get("schemaVersion") == "maiden-kum-trainer-manifest-v1",
        manifest.get("candidateVersion") == EXPECTED_VERSION,
        manifest.get("live") is False,
        manifest.get("usedForRanking") is False,
        manifest.get("telegramVisible") is False,
        manifest.get("observationStart") == EXPECTED_OBSERVATION_START,
        manifest.get("baselineVersion") == EXPECTED_BASELINE_VERSION,
        manifest_scope.get("category") == "MAIDEN",
        str(manifest_scope.get("track") or "").casefold() == "kum",
        manifest.get("metric") == EXPECTED_METRIC,
        manifest_formula.get("rawWeightAddPoints") == EXPECTED_RAW_ADD_POINTS,
        manifest_formula.get("baselineTemperature") == EXPECTED_BASELINE_TEMPERATURE,
        manifest_formula.get("candidateTemperature") == EXPECTED_CANDIDATE_TEMPERATURE,
        manifest_audit.get("artifact") == EXPECTED_AUDIT_ARTIFACT,
        manifest_audit.get("artifactSha256") == EXPECTED_AUDIT_SHA256,
        manifest_audit.get("sourceSnapshotSha256") == EXPECTED_SOURCE_SNAPSHOT_SHA256,
        manifest_audit.get("strictValidDate") is True,
        manifest_audit.get("historicalOuterUntouched") is False,
        manifest_audit.get("prospectiveEvidenceStartsAtZero") is True,
        manifest_overlap.get("nonAdditive") is True,
        manifest.get("checkpoints") == list(CHECKPOINTS),
        manifest_gates.get("primaryObjective") == "winner_top3",
        manifest_gates.get("winnerTop3NetMinAtCheckpoint") == 0,
        manifest_gates.get("winnerTop3NetMinAtFormal") == 1,
        manifest_gates.get("damageMax") == 0,
        manifest_gates.get("top1EarlyCheckpointVeto") is False,
        manifest_gates.get("top1FormalLossMax") == 1,
        manifest_gates.get("objectiveDeltaMin") == -0.002,
        manifest_gates.get("maeDeltaMax") == 0.05,
        manifest_gates.get("rhoDeltaMin") == -0.01,
        manifest_gates.get("ndcg5DeltaMin") == -0.002,
    )
    if not all(audit_checks):
        return {**current, "reason": "audit_artifact_identity_mismatch"}
    if not all(manifest_checks):
        return {**current, "reason": "manifest_artifact_identity_mismatch"}
    return {
        **current,
        "ready": True,
        "reason": None,
        "splitRaces": {"build": 19, "inner": 6, "outer": 7},
        "temperatureFit": {
            "baseline": EXPECTED_BASELINE_TEMPERATURE,
            "candidate": EXPECTED_CANDIDATE_TEMPERATURE,
            "fitSplit": "historical_build",
        },
        "ml15OverlapNonAdditive": True,
    }
...
def calibration_summary(races: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence": evidence,
        "fitScope": "historical_build_only" if evidence_ready else None,
        "sourceSnapshotSha256": (
            EXPECTED_SOURCE_SNAPSHOT_SHA256 if evidence_ready else None
        ),
...
def formal_support_ready(
    ...
    *,
    ...
    rank_quality_gate: bool,
    calibration_gate: bool,
    integrity_invalid_races: int,
    versions: set[str],
) -> bool:
    """Apply the +15 formal-replay ceiling with Winner Top3 priority."""
    return bool(
        complete_count >= 15
        ...
        and source_gate
        and separation_gate
        and rank_quality_gate
        and calibration_gate
        and integrity_invalid_races == 0
...
def evidence_rankings(
    ...
    replay_order = sorted(
        ...
        key=lambda row: (
            -safe_float(
                row.get("maiden_kum_trainer_candidate_replay_baseline_score"),
                0.0,
            ),
    ...
    candidate_order = sorted(
        competitive,
        key=lambda row: (
            -safe_float(row.get("maiden_kum_trainer_candidate_score"), 0.0),
            safe_int(row.get("rank_pred"), 999),
            str(row.get("horse_name") or ""),
        ),
    )
    ...
    for row in competitive:
        ...
        evidence_rows.append({
            **row,
            "maiden_kum_trainer_evidence_visible_rank": safe_int(row.get("rank_pred"), 999),
            "maiden_kum_trainer_evidence_replay_baseline_rank": replay_rank[name_key],
            "maiden_kum_trainer_evidence_candidate_rank": candidate_rank[name_key],
        })
    visible_top3 = {
        str(row.get("horse_name") or "").casefold()
        for row in sorted(
            evidence_rows,
            key=lambda row: safe_int(row.get("maiden_kum_trainer_evidence_visible_rank"), 999),
        )[:3]
    }
    replay_top3 = {
        str(row.get("horse_name") or "").casefold()
        for row in sorted(
            ...
            key=lambda row: safe_int(
                row.get("maiden_kum_trainer_evidence_replay_baseline_rank"),
                999,
            ),
        )[:3]
    }
...
def build_report(entries: list[dict[str, Any]], run_date: str) -> dict[str, Any]:
    ...
    for entry in entries:
        version = str(entry.get("maiden_kum_trainer_candidate_version") or "").strip()
        if version:
            grouped[(
    ...
    for ((_, _, version), rows) in sorted(grouped.items(), key=lambda item: race_sort_key(item[1])):
        ...
        race_day = parse_race_date(rows[0].get("race_date"))
        start_day = parse_race_date(rows[0].get("maiden_kum_trainer_candidate_observation_start"))
        ...
        if not replay_top3_set_agreement:
            ...
            visible_top3 = [
                ...
                for row in sorted(
                    evidence_rows,
                    key=lambda row: safe_int(
                        row.get("maiden_kum_trainer_evidence_visible_rank"),
                        999,
                    ),
                )[:3]
            ]
        ...
        source_rows = [
            row.get("maiden_kum_trainer_candidate_source") or {}
        ...
        baseline_separation = separation(
            evidence_rows,
            "maiden_kum_trainer_evidence_replay_baseline_rank",
            "maiden_kum_trainer_candidate_replay_baseline_score",
        )
        candidate_separation = separation(
            evidence_rows,
            "maiden_kum_trainer_evidence_candidate_rank",
            "maiden_kum_trainer_candidate_score",
        )
        complete.append({
            ...
            "candidateCreatedTs": safe_int(winner.get("maiden_kum_trainer_candidate_created_ts"), 0),
            "profile": winner.get("maiden_kum_trainer_candidate_profile"),
            "definitionSha256": winner.get("maiden_kum_trainer_candidate_definition_sha256"),
            "raceSnapshotSha256": winner.get("maiden_kum_trainer_candidate_race_snapshot_sha256"),
            "visibleWinnerRank": safe_int(
                winner.get("maiden_kum_trainer_evidence_visible_rank"),
                0,
            ),
            "baselineWinnerRank": safe_int(
                winner.get("maiden_kum_trainer_evidence_replay_baseline_rank"),
                0,
            ),
            "candidateWinnerRank": safe_int(
                winner.get("maiden_kum_trainer_evidence_candidate_rank"),
                0,
            ),
            ...
            "winnerSourceActionable": bool(
                (winner.get("maiden_kum_trainer_candidate_source") or {}).get("actionable")
            ),
            ...
            "scoreChangedCount": sum(
                abs(
                    safe_float(row.get("maiden_kum_trainer_candidate_score"), 0.0)
                    - safe_float(row.get("maiden_kum_trainer_candidate_replay_baseline_score"), 0.0)
                ) > 1e-9
                for row in evidence_rows
            ),
        })
    ...
    rank_quality = bool(
        ...
        and cumulative["objectiveDelta"] >= -0.002
        and cumulative["candidate"]["mae"] is not None
        and cumulative["baseline"]["mae"] is not None
        and cumulative["candidate"]["mae"] <= cumulative["baseline"]["mae"] + 0.05
        and cumulative["candidate"]["rho"] is not None
        and cumulative["baseline"]["rho"] is not None
        and cumulative["candidate"]["rho"] >= cumulative["baseline"]["rho"] - 0.01
        and cumulative["candidate"]["ndcg5"] is not None
        and cumulative["baseline"]["ndcg5"] is not None
        and cumulative["candidate"]["ndcg5"] >= cumulative["baseline"]["ndcg5"] - 0.002
    )
    formal_supported = formal_support_ready(
        cumulative,
        checkpoints,
        complete_count=len(complete),
        source_gate=source_gate,
        separation_gate=separation_quality["passed"],
        rank_quality_gate=rank_quality,
        calibration_gate=cumulative["calibration"]["passed"],
        integrity_invalid_races=coverage["integrityInvalidRaces"],
        versions=versions,
    )
    ...
    return {
        ...
        "primaryGate": {
            "winnerTop3NetMinAtCheckpoints": 0,
            "winnerTop3NetMinAtFormal": 1,
            "damageMax": 0,
            "nonCausalRescueMax": 0,
        },
        "top1Policy": {
...
def markdown(report: dict[str, Any]) -> str:
    ...
    baseline, candidate = summary["baseline"], summary["candidate"]
    lines = [
        f"# MAIDEN Kum Trainer +2 Shadow - {report['runDate']}",
        "",
        ...
        f"| Trainer score +2 | {candidate['top1']} | {candidate['winnerTop3']} | "
        ...
        "- Primary objective: Winner Top3; Top1 has no early-checkpoint veto and "
        "only a >1 loss blocks formal replay.",
...
def persist(report: dict[str, Any], data_dir: Path) -> None:
    ...
    latest_dir = data_dir / "automation" / "maiden-kum-trainer-shadow"
    encoded = json.dumps(report, ensure_ascii=False, indent=2) + "\n"
    rendered = markdown(report)
    for path, content in (
        (run_dir / "maiden-kum-trainer-shadow-checkpoint.json", encoded),
        (run_dir / "maiden-kum-trainer-shadow-checkpoint.md", rendered),
